...
class ASFF(nn.Module):
    def __init__(self, level, rfb=True, vis=False):
        ...
        self.level = level
        self.dim = [512, 256, 128] #yolov4s
        #self.dim = [1024,512,256] #yolov4l
        self.inter_dim = self.dim[self.level]
        if level==0:
            self.stride_level_1 = add_conv(256, self.inter_dim, 3, 2)
            self.stride_level_2 = add_conv(128, self.inter_dim, 3, 2)
            self.expand = add_conv(self.inter_dim, 24, 3, 1)
        elif level==1:
    ...
    def forward(self, x):
        x_level_0=x[0] #512,19x19
        x_level_1=x[1] #256,38x38
        x_level_2=x[2] #128,76x76
        #print(x_level_0.size())
        if self.level==0:
            level_0_resized = x_level_0
            level_1_resized = self.stride_level_1(x_level_1)
...
if __name__=='__main__':
    model=ASFF(1)
    input1=torch.randn(1,512,19,19)
    input2=torch.randn(1,256,38,38)
    input3=torch.randn(1,256,76,76)
